...
// Bloqueia /dashboard/agenda pra contas sem o serviço "agenda" contratado —
// além de escondido no menu (DashboardShell), pra quem tentar acessar a URL
// direto. Admin de plataforma vê tudo, a não ser que tenha ligado "Ver como
// cliente" (mesmo padrão do gate de conexão do WhatsApp em DashboardShell).
export default async function AgendaLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isPlatformAdmin || viewingAsClient) {
    ...
    if (session?.user?.contaId) {
      ...
      if (!temServico(conta?.servicosContratados, 'agenda')) {
        redirect('/dashboard')
      }
    }
  }
