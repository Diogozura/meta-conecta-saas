  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {users.length === 0 && <p className="text-sm text-ink-500">Nenhum usuário cadastrado.</p>}
        {users.map((user) =>
          editingId === user.id ? (
            <div key={user.id} className="border border-brand-300 rounded-lg p-3 space-y-2">
              <input
                type="text"
                value={editName}
            ...
            <div key={user.id} className="border border-ink-200 rounded-lg p-3 flex items-center justify-between gap-3">
      ...
      {lastPageWasFull && (
        <button
          ...
          className="w-full px-4 py-2 border border-ink-300 text-ink-700 text-sm font-medium rounded-lg hover:bg-ink-50 disabled:opacity-50 transition-colors"
        >
