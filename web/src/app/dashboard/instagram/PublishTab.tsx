...
export default function PublishTab({ connected }: { connected: boolean }) {
  ...
  async function handleDeleteHistoryItem(id: string) {
    if (!window.confirm('Remover esta publicação do histórico do painel? O post publicado no Instagram não é afetado.')) return
    setDeletingId(id)
    try {
            ...
            {publicacoes.map((p) => (
              <div key={p.id} className="flex items-center gap-3 p-3">
                ...
                <button
                  ...
                  className="p-1.5 text-ink-400 hover:text-red-600 disabled:opacity-50 shrink-0"
                  aria-label="Remover do histórico"
                  title="Remover do histórico (não apaga do Instagram)"
                >
